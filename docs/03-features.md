...
Tutto è centralizzato in un unico **View Composer** registrato in `App\Providers\AppServiceProvider::boot()`. Il composer è agganciato a `*` (tutte le view) perché il menu della sidebar è renderizzato da AdminLTE su ogni richiesta, prima della view di pagina.
