...
- **Categorie** — CRUD con slug auto-generato; gestione **materiale didattico** per categoria (PDF, link/YouTube, note testuali) con drag-and-drop per il riordinamento (`admin/categories/{id}/materials`); flag `is_eu_directive` per marcare le categorie del percorso EU (filtrabile con `eu_categories_visible`); **traduzioni del nome categoria** (`admin/categories/{category}/edit` → sezione "Traduzioni"): admin/editor caricano il nome nella lingua configurata (lingue UI supportate; IT escluso come fonte di verità). `Category::getLocalizedName(string $locale)` restituisce la traduzione con fallback automatico all'italiano. 15 test in `CategoryTranslationTest`
- **Contenuti formativi — StudyContent** (`admin/study-contents`) — CRUD di contenuti HTML didattici collegati polimorfico a una **categoria EU** o a un **modulo di guida pratica**. Editor TinyMCE 6 (caricato solo in queste view) con integrazione Media Manager via iframe/postMessage. Ogni contenuto ha flag `is_published` e ordinamento `order`. Observer cascade: cancellare la categoria/modulo elimina i contenuti collegati (polimorfismo non supporta FK cascade DB). Gate: `canEditStudyContent()` (admin/editor su tutto; instructor solo su `DrivingModule`). I toggle `study_content_enabled` e `eu_categories_visible` in `/admin/system/features` controllano l'abilitazione runtime. 9 test in `StudyContentTest`
...
- **Contenuti formativi** — nella pagina dettaglio di una categoria EU (`admin/categories/{id}/show`) e in quella di un modulo di guida pratica (`admin/driving-modules/{id}/show`) compare il componente Livewire `StudyContentViewer`: mostra i contenuti HTML pubblicati per quell'entità con pulsante **"Segna come letto"** (`wire:loading` su azione). Empty state con icona `fa-3x`. Visibile se il toggle `study_content_enabled` è abilitato
