...
- **Homepage guest personalizzabile (Feature 11.1)** — landing page per visitatori non autenticati con navbar, hero (gradiente `--sg-accent`), sezione statistiche, feature highlights, badge tipi di patente e CTA finale. Tutti i testi della scuola (nome, tagline, logo) vengono da `setting()` — l'admin personalizza senza toccare il codice. Layout `layouts/guest.blade.php` separato da AdminLTE, dark mode automatica via `prefers-color-scheme`, i18n IT/EN/ES.
...
- **Guide pratiche — moduli e sessioni** — l'admin configura i moduli di guida pratica per tipo di patente (codice, ore richieste, ordine MIT); istruttori e admin registrano le sessioni dei singoli studenti; ogni viewer consulta il proprio avanzamento (ore completate / richieste per modulo, percentuale globale). Il calcolo avanzamento avviene in due query (moduli + sessioni raggruppate) senza N+1. Il modulo non è eliminabile finché esistono sessioni registrate.
- **Export PDF attestazione guide pratiche** — il viewer completo scarica un riepilogo PDF delle proprie sessioni quando ha terminato tutte le ore obbligatorie; l'istruttore e l'admin possono esportare il PDF per qualsiasi studente. Il documento contiene intestazione autoscuola (nome, indirizzo, contatti, n. autorizzazione MIT, configurabili tramite variabili `.env`), dati studente, riepilogo avanzamento per modulo, dettaglio sessioni, elenco istruttori. Il file PDF è generato on-demand e rimosso dal disco al download. Cleanup automatico notturno alle 03:30 per i file non scaricati (fallback di sicurezza).
