...
- **Homepage guest personalizzabile (Feature 11.1 + redesign 15.3)** — landing page per visitatori non autenticati con navbar, hero con carosello immagini come sfondo (max 4 slide, 1920×600 px, gestibili da `admin/system/settings`), sezione statistiche, feature highlights, badge tipi di patente e CTA finale. Tutti i testi della scuola (nome, tagline, logo) vengono da `setting()`. Struttura hero "sobria e ariosa" (`sg-hero`): sfondo carosello o accent solido con overlay gradiente unico; titolo/tagline/bottoni direttamente sul bg, nessun box annidato (redesign 15.3). Feature card con hover chip e micro-interazioni discrete. Layout `layouts/guest.blade.php` separato da AdminLTE (Bootstrap 5.3 + Alpine via CDN), i18n IT/EN/ES, light/dark mode via `data-bs-theme`. Richiede `php artisan db:seed --class=SystemSettingSeeder` al primo setup.
- **Pannello sistema — stato servizi e personalizzazione (Feature 11.0)** — pannello admin con due sezioni: *Stato servizi* (6 indicatori live: Database, Redis, Queue, Storage, Mail, Twilio) e *Personalizzazione* (nome scuola, tagline, indirizzo, contatti, n. autorizzazione MIT, logo chiaro/dark, carosello homepage). I dati della scuola sono la fonte di verità per tutte le view tramite l'helper `setting('school.name')`; il logo viene servito via `Storage::url()`. Dal lotto 15.0 i colori, il font e il border-radius sono costanti del design system in `scuola-guida.css` e non sono più modificabili dal pannello.
...
L'applicazione è ampiamente configurabile senza toccare il codice: variabili
d'ambiente, file `config/`, pannelli di sistema e comandi artisan eseguibili
dall'interfaccia admin.
...
- **`/admin/system/settings`** — dati autoscuola (nome, tagline, indirizzo, contatti, logo, carosello) persistiti su `system_settings`. I colori (palette, accent) e il font sono costanti del design system (`public/css/scuola-guida.css`) e non sono configurabili dall'admin (da Feature 15.0). Sidebar e navbar usano uno shell navy uniforme per tutti i ruoli (Feature 15.1): il colore di ruolo compare come striscia laterale, voce attiva e badge pill. L'area guest e auth usa il medesimo design system "sobrio contemporaneo" con token `sg-*` (Feature 15.3).
...
- **`/admin/commands`** — esecuzione comandi artisan whitelist (code, cache, sistema, backup, GDPR)

Per l'elenco completo di ogni variabile, chiave e pannello vedi
**[docs/11-configuration.md](docs/11-configuration.md)**.
...
| [docs/11-configuration.md](docs/11-configuration.md) | Configurazione completa: variabili `.env`, file `config/`, pannelli di sistema (settings, form-fields, features, health, commands) |
