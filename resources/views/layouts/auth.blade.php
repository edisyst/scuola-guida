...
<body class="guest-page" style="background:#f4f6f9;">
    ...
    {{-- Navbar minimale (identica alla homepage guest) --}}
            ...
            <div class="sg-auth-brand">
                @if(setting('school.logo_path'))
                    <a href="{{ route('guest.home') }}"
                       title="{{ setting('school.name', config('app.name')) }}">
                        <img src="{{ Storage::url(setting('school.logo_path')) }}"
                             alt="{{ setting('school.name', config('app.name')) }}"
                             style="max-height:56px;width:auto;">
                    </a>
                @else
                    <a href="{{ route('guest.home') }}" class="sg-auth-logo"
                       title="{{ setting('school.name', config('app.name')) }}">
                        <i class="fas fa-car"></i>
                    </a>
                @endif
                <span class="sg-auth-app-name">{{ setting('school.name', config('app.name')) }}</span>
            </div>
    ...
    {{-- Footer --}}
    <footer class="py-4 bg-light text-muted">
